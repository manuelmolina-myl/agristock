-- ============================================================================
-- 025_simplify_roles_data_and_policies.sql (part 2 of 2)
-- Runs after 024 has committed the new enum values.  Performs:
--   1. Re-grant existing user_roles rows under the new 4-value naming.
--   2. Rewrite helpers (current_user_roles, auth_role, can_write_*, RPCs).
--   3. Tighten profiles.role CHECK + sync trigger.
--   4. Rewrite RLS policies that referenced legacy role names.
--   5. Update organization settings approval_thresholds.
-- ============================================================================

-- ─── 1. Re-grant user_roles rows under the new naming ──────────────────────
-- 1a. Anything that meant "full access" → admin
insert into public.user_roles (organization_id, user_id, role)
select organization_id, user_id, 'admin'::user_role
  from public.user_roles
 where role in ('super_admin', 'director_sg', 'auditor')
   and revoked_at is null
on conflict (organization_id, user_id, role) do nothing;

-- 1b. coordinador_compras + solicitante → compras
insert into public.user_roles (organization_id, user_id, role)
select organization_id, user_id, 'compras'::user_role
  from public.user_roles
 where role in ('coordinador_compras', 'solicitante')
   and revoked_at is null
on conflict (organization_id, user_id, role) do nothing;

-- 1c. coordinador_mantenimiento + tecnico + operador → mantenimiento
insert into public.user_roles (organization_id, user_id, role)
select organization_id, user_id, 'mantenimiento'::user_role
  from public.user_roles
 where role in ('coordinador_mantenimiento', 'tecnico', 'operador')
   and revoked_at is null
on conflict (organization_id, user_id, role) do nothing;

-- 1d. Revoke legacy rows so current_user_roles() returns only the new set.
update public.user_roles
   set revoked_at = now()
 where role in (
   'super_admin', 'director_sg', 'coordinador_compras', 'coordinador_mantenimiento',
   'tecnico', 'operador', 'solicitante', 'auditor'
 )
 and revoked_at is null;
...
as $$
  ...
  order by case role
    ...
    when 'compras'::user_role        then 2
    when 'mantenimiento'::user_role  then 3
    when 'almacenista'::user_role    then 4
    else 99
  end;
$$;

-- auth_role: map new enum back to legacy string so RLS in 001_schema.sql
-- ('super_admin'/'gerente'/'almacenista'/'supervisor') keeps working.
...
as $$
  select case role
    when 'admin'::user_role          then 'super_admin'
    when 'compras'::user_role        then 'supervisor'
    when 'mantenimiento'::user_role  then 'supervisor'
...
as $$
  select exists (
    ...
    where user_id = p_user_id
      and role in ('admin'::user_role, 'compras'::user_role)
      and revoked_at is null
  );
$$;
...
as $$
  select exists (
    select 1 from public.user_roles
    where user_id = p_user_id
      and role in ('admin'::user_role, 'mantenimiento'::user_role)
      and revoked_at is null
  );
$$;

-- ─── 3. profiles.role: tighten to the new 4 values + sync trigger ──────────
...
-- ─── 4. Update organization settings approval tiers ────────────────────────
...
-- ─── 6. Update RPCs that named legacy roles in their bodies ────────────────
