...
const formSchema = z.object({
  ...
  fx_rate:            z.coerce.number().positive('Tipo de cambio inválido'),
...
export default function NuevaEntradaPage() {
  ...
  const watchedFx    = form.watch('fx_rate') ?? latestFxRate
  ...
  async function goNext() {
    let valid = false
    if (step === 1) {
      valid = await form.trigger(['movement_type', 'warehouse_id', 'movement_date', 'fx_rate'])
    } else if (step === 2) {
    ...
    try {
      ...
      const { data: movement, error: movErr } = await (supabase as any)
        ...
        .insert({
          ...
          total_usd:          grandTotalMxn / (values.fx_rate ?? 1),
      ...
      const linePayloads = values.lines.map((line, i) => {
        ...
        const unitMxn     = currency === 'MXN' ? line.unit_cost_native : line.unit_cost_native * (values.fx_rate ?? 1)
        const totalMxn    = currency === 'MXN' ? totalNative : totalNative * (values.fx_rate ?? 1)
        return {
        ...
        <Card>
          ...
          <CardContent className="grid gap-5">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              {/* FX rate */}
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="fx_rate">Tipo de cambio USD/MXN <span className="text-destructive">*</span></Label>
                <Input
                  id="fx_rate"
                  type="number"
                  step="0.0001"
                  ...
                  {...form.register('fx_rate')}
                />
                <FieldError message={form.formState.errors.fx_rate?.message} />
              </div>
            </div>
            ...
            <CardContent className="flex flex-col gap-0">
              {[
                ...
                },
                {
                  ...
                  value: <span className="font-mono">{form.getValues('fx_rate')?.toFixed(4)}</span>,
                },
            ...
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col items-end gap-1">
                  <MoneyDisplay amount={grandTotalMxn} currency="MXN" />
                  <MoneyDisplay
                    amount={grandTotalMxn / (form.getValues('fx_rate') ?? 1)}
                    currency="USD"
                  />
                </div>
              </div>
