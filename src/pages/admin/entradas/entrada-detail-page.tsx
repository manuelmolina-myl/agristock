...
export default function EntradaDetailPage() {
  ...
  const { data: mov, isLoading } = useQuery<MovementDetail>({
    ...
    queryFn: async () => {
      const { data, error } = await db
        ...
        .select('*, warehouse:warehouses!stock_movements_warehouse_id_fkey(name, code), supplier:suppliers(name), received_by:employees!stock_movements_received_by_employee_id_fkey(full_name), lines:stock_movement_lines(*, item:items(sku, name, unit:units(code)))')
        .eq('id', id)
        .single()
  ...
  if (!mov) {
    return (
      <div className="flex flex-col gap-6 p-4 sm:p-6">
        <Button variant="ghost" size="sm" className="w-fit -ml-1" onClick={() => navigate(`${basePath}/entradas`)}>
          <ArrowLeft className="mr-1.5 size-3.5" /> Entradas
        </Button>
        <EmptyState title="Entrada no encontrada" description="El movimiento no existe o fue eliminado." action={{ label: 'Volver a Entradas', onClick: () => navigate(`${basePath}/entradas`) }} />
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-4 sm:p-6">
      {/* Back */}
      <Button variant="ghost" size="sm" className="w-fit -ml-1 text-muted-foreground" onClick={() => navigate(`${basePath}/entradas`)}>
        <ArrowLeft className="mr-1.5 size-3.5" /> Entradas
      </Button>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {mov.status === 'posted' && (
            <Button variant="outline" size="sm" onClick={handlePrint}>
              <Printer className="mr-1.5 size-3.5" /> Imprimir nota
            </Button>
          )}
          {mov.status === 'draft' && (
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    {canSeePrices && <TableHead className="w-32 text-right">Costo unit.</TableHead>}
                    ...
                    <TableRow key={line.id}>
                      ...
                      {canSeePrices && (
                        <TableCell className="text-right">
                          <MoneyDisplay amount={line.unit_cost_mxn} currency="MXN" />
                        </TableCell>
                      )}
                      {canSeePrices && (
                        <TableCell className="text-right">
